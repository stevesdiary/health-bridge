...
export const paymentService = {
  initiatePayment: async (paymentData: PaymentRequestData): Promise<PaymentResponse> => {
    try {
      ...
      console.log("PATIENT DATA FROM USER", paymentData.email, patient?.dataValues.patient.id);
      if (!patient) {
        return {
          statusCode: 404,
          status: 'error',
      ...
      const appointment = await Appointment.findOne({
        where: {
          patient_id: patient?.dataValues.patient.id,
          status: 'pending'
        },
      ...
      const createPayment = await Payment.create({
        ...
        currency: paymentData.currency || 'NGN',
        patientId: patient.id,  // Use the patient_id from payment data
